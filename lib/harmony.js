...
class HarmonyNode extends EventEmitter {
  ...
  async route(message) {
    ...
    try {
      ...
      if (!recipient) {
        console.warn(
          `⚠️  Message dropped: ${message.recipient} not found. Available: ${Array.from(this.modules.keys()).join(', ')}`
        );
        this.emit('message:dropped', { message, reason: 'Recipient not found' });
        return;
      }
      ...
      if (!recipient.endpoints[message.action]) {
        console.warn(
          `⚠️  Message dropped: ${message.recipient} does not have endpoint: ${message.action}`
        );
        this.emit('message:dropped', { message, reason: 'Endpoint not found' });
        return;
      }
