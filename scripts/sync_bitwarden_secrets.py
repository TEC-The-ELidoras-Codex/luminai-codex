...
class BitwardenSecretsManager:
    ...
    def __init__(self, vault_name: str = "luminai-codex", base_path: str = None):
        ...
        self.base_path = (
            Path(base_path) if base_path else Path("/home/tec_tgcr/luminai-codex")
        )
    ...
    def check_bw_cli(self) -> bool:
        ...
        try:
            ...
            logger.error(
                "Bitwarden CLI not installed. Install with: npm install -g @bitwarden/cli"
            )
            return False
    ...
    def sync_secrets_to_env(self, environment: str, items: List[Dict]) -> bool:
        ...
        try:
            ...
            backup_file = (
                self.config_path
                / f".env.{environment}.backup.{datetime.now().strftime('%Y%m%d_%H%M%S')}"
            )
...
def main():
    ...
    parser.add_argument(
        "--env", choices=["dev", "staging", "prod"], help="Environment to synchronize"
    )
    parser.add_argument("--vault", default="luminai-codex", help="Bitwarden vault name")
    parser.add_argument(
        "--sync-all", action="store_true", help="Synchronize all environments"
    )
    parser.add_argument("--rotate-secrets", action="store_true", help="Rotate secrets")
    parser.add_argument(
        "--base-path", help="Base path for the project (default: current directory)"
    )

    args = parser.parse_args()

    # Initialize the secrets manager
    manager = BitwardenSecretsManager(vault_name=args.vault, base_path=args.base_path)
