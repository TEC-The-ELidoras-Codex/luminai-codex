...
To set up your local environment, copy `.env.example` to `.env.local` and populate the secrets locally (do not commit `.env.local`).

Example placeholders (replace locally, do not commit):
