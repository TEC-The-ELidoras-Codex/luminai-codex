...
| Pre-commit secret scanning | ⏳ Pending (recommended) |
| History purge | ⏳ Optional (strongly advised) |

## Recommended Actions (Priority Order)
1. (Done) Rotate all keys appearing in committed log.  
2. Implement secret scanning pre-commit (e.g., `.git/hooks/pre-commit` using `gitleaks` or simple regex).  
3. Run CI secret scanning via GitHub Advanced Security or `gitleaks` action.  
