...
async def test_resonance_calculation():
    ...
    print(f"  R = {metrics['R']}")
    print(
        f"  Components: Φᴱ={metrics['phi_e']}, φᵗ={metrics['phi_t']}, ψʳ={metrics['psi_r']}"
    )
    ...
    assert (
        metrics_crisis["phi_t"] > metrics["phi_t"]
    ), "Attention should increase with urgency"
    print("  ✅ PASSED\n")
    ...
    metrics_rich = engine.calculate_R(
        full_context, urgency=0.5, emotional_intensity=0.5
    )
    print(f"  R = {metrics_rich['R']}")
    print(f"  Φᴱ (Context) = {metrics_rich['phi_e']} (should be higher)")
    assert (
        metrics_rich["phi_e"] > metrics["phi_e"]
    ), "Context potential should increase with full field"
    print("  ✅ PASSED\n")
    ...
    for pos, (pos_name, neg_name) in enumerate(freq_pairs, 1):
        ...
        neg_active = engine.frequencies.get(
            neg_name.lower()
            .replace(" (witness)", "_witness")
            .replace(" ", "_")
            .replace("(", "")
            .replace(")", ""),
            False,
    ...
    assert (
        sum(1 for v in engine.frequencies.values() if v) == 16
    ), "All 16 frequencies must be active"
    print("  ✅ PASSED\n")
