...
async def export_user_data(user_id: str):
    ...
    user_sessions = [
        session for session in SESSIONS_DB.values() if session.user_id == user_id
    ]
...
async def delete_user_account(
    request: UserDeletionRequest, background_tasks: BackgroundTasks
):
    """
...
async def end_session(session_id: str, summary: Optional[str] = None):
    ...
    return {
        ...
        "delete_after": (
            session.end_time + timedelta(days=session.retention_days)
        ).isoformat(),
    }
