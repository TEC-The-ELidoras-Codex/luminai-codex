...
async def get_multi_llm_response(request: MultiLLMRequest) -> MultiLLMResponse:
    ...
    if not api_keys[persona]:
        raise HTTPException(
            status_code=500, detail=f"API key not configured for {persona}"
        )
    ...
    return MultiLLMResponse(
        ...
        persona=persona,
        model={"claude": "claude-3-opus", "openai": "gpt-4-turbo", "xai": "grok-1"}[
            persona
        ],
...
async def save_conversation(
    conversation_id: str, messages: List[Message], resonance_score: float
):
...
async def get_conversation(conversation_id: str):
    ...
    return {"conversation_id": conversation_id, "messages": [], "resonance_score": 0.84}


@router.post("/conversation/export")
async def export_conversation(
    conversation_id: str, format: Literal["markdown", "pdf", "json"] = "markdown"
):
