...
class AirthResearchGuard:
        ...
        try:
            ...
            for check_result in results:
                if check_result["violations"]:
                    ...
                    guard_result["recommendations"].extend(
        ...
        try:
            ...
            async with httpx.AsyncClient() as client:
                response = await client.post(
                    "http://localhost:11434/api/generate", json=payload, timeout=30.0
                )
            ...
            return {"source": "ollama", "error": str(e), "confidence": 0.0}
    ...
    async def _apply_guard_checks(self, research_result: Dict) -> Dict:
        ...
        return {"status": "pass", "violations": [], "recommendations": []}
    ...
    def respond(self, query: str, tools: Optional[List] = None) -> str:
        ...
        if "schedule" in query.lower():
            ...
            "branding knowledge" in query.lower()
            or "knowledge branding" in query.lower()
        ):
            ...
            "tools": [
                {
                    ...
                    "description": "Look up knowledge and branding information",
                },
                {
                    "name": "schedule_planner",
                    "description": "Plan and manage schedules",
                },
                {
                    "name": "sharepoint_publish",
                    "description": "Publish content to SharePoint",
                },
                {
                    "name": "spotify_resonance",
                    "description": "Analyze Spotify track resonance",
                },
                {
                    "name": "llm_responder",
                    "description": "Generate LLM responses and synthesis",
                },
                {
                    "name": "web_research",
                    "description": "Conduct web research and fact-checking",
                },
            ],
            "capabilities": [
                "research",
